...
str_1 = "\n".join(f"{key}={value}" for key, value in dict_args.items())
...
if __name__ == '__main__':
    ...
    for epoch in range(args.epochs):
        ...
        for step, (exocentric_image, egocentric_image, aff_label) in enumerate(TrainLoader):
            aff_label = aff_label.cuda(non_blocking=True).long()  # b x n x 36
            exo = exocentric_image.cuda(non_blocking=True)  # b x n x 3 x 224 x 224
            ego = egocentric_image.cuda(non_blocking=True)
            ...
            exo_aff_loss = nn.CrossEntropyLoss().cuda()(exo_aff_logits.view(-1, exo_aff_logits.size(-1)), aff_label.repeat_interleave(num_exo))
